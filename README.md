...
**在线页面**：<https://zyxzyxzyx.github.io/zcode-usage/>（GitHub Pages 静态快照，本地每 1 分钟自动推送、页面每 60 秒自动拉取）

**桌面悬浮框（v2.0）**：双击 `启动悬浮框.bat`，屏幕右上角常驻一张置顶悬浮卡，实时显示火山方舟网关剩余额度与百分比（悬停展开明细、托盘图标、右键菜单，详见下文）。

**零依赖、零侵入**：只读 ZCode 本地 SQLite 数据库，不写 ZCode 任何文件，不需要 Docker / 数据库 / npm install。

## 桌面悬浮框（v2.0）

- 形态：260px 宽透明置顶小卡（默认右上角、85% 不透明度），显示方舟网关剩余 tokens / 百分比 / 迷你进度条 / 重置时间
- 交互：拖动移动（位置记忆）、悬停展开分模型明细、双击打开完整仪表盘、右键 ⋮ 菜单（置顶/不透明度/隐藏到托盘/退出）、托盘常驻（环形进度图标 + tooltip 百分比）
- 数据：v1.0 本地服务的 SSE 实时推送，断线自动退化为 2 秒轮询；服务未运行时卡片变灰并提供「启动服务」按钮
- 运行：`启动悬浮框.bat`（首次自动 `npm install` Electron）；技术栈 Electron，代码在 `widget/`，设置存 `data/widget.json`
